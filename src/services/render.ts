...
function layout(title: string, active: string, body: string) {
  const nav = [
    { href: "/", label: "Overview" },
    { href: "/claim-audit", label: "Claim Audit" },
    { href: "/proof-gaps", label: "Proof Gaps" },
    { href: "/board-readout", label: "Board Readout" },
...
<html lang="en">
  ...
  <body>
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Claims vs Reality Engine</div>
        <div class="right">
          <div>board-ready executive intelligence · synthetic sample data only</div>
          <div>claim audit · proof gap scoring · diligence memo</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            ...
            <span class="meta-chip">Claims versus proof scorecard</span>
            <span class="meta-chip">Synthetic sample data only</span>
          </div>
          <h1>Claim-versus-proof intelligence that shows which stories are real, which are stale, and what belongs in the board memo.</h1>
          <p>This engine turns AI readiness claims, identity posture, savings narratives, compliance language, and platform promises into one executive surface: score, proof gaps, replacement signals, and diligence-ready board packets.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Board-ready claim-audit intelligence for executives, investors, and operating partners.</strong><br />One surface for where marketing, procurement, and executive language outruns actual proof.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer plus executive control surface.</strong><br />This repo reads synthetic vendor and platform claim reviews and turns them into score, proof gaps, board-readout packets, and diligence-safe outputs.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Leaders looking for <strong>vendor claim audits, board-ready diligence, replacement cases, savings proof, AI-readiness scoring, and executive evidence posture</strong> should see a real product, not positioning theater.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>claims-vs-reality-engine · synthetic sample data only</div>
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Executive Snapshot</h2><div class="note">score · contradiction · savings</div></div>
        <div class="kpis">
          <div class="kpi"><div class="v">${metrics.evidenceScore}</div><div class="lbl">evidence score</div><div class="h">Rollup score for proof freshness, contradiction risk, and board readiness.</div></div>
          <div class="kpi"><div class="v">${metrics.vendors}</div><div class="lbl">vendors</div><div class="h">Named products or vendors modeled in the executive claim review view.</div></div>
          <div class="kpi"><div class="v">${metrics.claims}</div><div class="lbl">claims</div><div class="h">Board or investor-facing narratives currently being tested against proof.</div></div>
          <div class="kpi"><div class="v">${metrics.contradictedClaims}</div><div class="lbl">contradicted</div><div class="h">Claims that materially outrun the evidence leadership could actually defend.</div></div>
          <div class="kpi"><div class="v">${metrics.staleClaims}</div><div class="lbl">stale</div><div class="h">Claims still leaning on stale or missing proof packs.</div></div>
          <div class="kpi"><div class="v">$${Math.round(metrics.replacementSavingsUsd / 1000)}k</div><div class="lbl">savings</div><div class="h">Modeled annual savings still trapped inside open replacement and proof work.</div></div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>What leaders need</h2><div class="note">exposure · invest · explain</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">where exposure sits</div><div class="src-tit">Keep the highest-risk narratives visible</div><p>${metrics.recommendation}</p></div>
          <div class="src"><div class="src-name">where to invest</div><div class="src-tit">Price proof gaps against board and CFO outcomes</div><p>Replacement cases, identity evidence, AI readiness, and compliance language now sit inside one executive scoring surface.</p></div>
          <div class="src"><div class="src-name">board story</div><div class="src-tit">Move from confident language to defensible language</div><p>Every lane resolves into proof gaps, a board-readout packet, and a clear remediation or investment move.</p></div>
        </div>
      </section>`
  );
}

export function renderClaimAudit() {
  return layout(
    "Claims vs Reality Engine — Claim Audit",
    "/claim-audit",
    `<section class="section">
        <div class="sh"><h2>Claim Audit</h2><div class="note">owner · contradiction · next action</div></div>
        <table class="ttbl">
          <thead><tr><th>Lane</th><th>Owner</th><th>Status</th><th>Related findings</th><th>Focus</th><th>Next action</th></tr></thead>
          <tbody>
            ${claimAudit().map((lane) => `<tr><td><b>${lane.lane}</b><br />${lane.note}</td><td>${lane.owner}</td><td><span class="st ${severityClass(lane.status)}">${lane.status}</span></td><td>${lane.relatedFindings}</td><td>${lane.focus}</td><td>${lane.nextAction}</td></tr>`).join("")}
          </tbody>
        </table>
      </section>`
  );
...
export function renderProofGaps() {
  return layout(
    "Claims vs Reality Engine — Proof Gaps",
    "/proof-gaps",
    `<section class="section">
        <div class="sh"><h2>Proof Gaps</h2><div class="note">severity · owner · executive subject</div></div>
        <table class="ttbl">
          <thead><tr><th>Claim</th><th>Owner</th><th>Vendor</th><th>Subject</th><th>Message</th></tr></thead>
          <tbody>
            ${proofGaps().map((finding) => `<tr><td><span class="st ${severityClass(finding.severity)}">${finding.severity}</span><br /><b>${finding.code}</b></td><td>${finding.owner}</td><td>${finding.vendor ?? "—"}</td><td>${finding.subjectName ?? finding.subject}</td><td>${finding.message}</td></tr>`).join("")}
          </tbody>
        </table>
      </section>`
  );
...
export function renderBoardReadout() {
  return layout(
    "Claims vs Reality Engine — Board Readout",
    "/board-readout",
    `<section class="section">
        <div class="sh"><h2>Board Readout</h2><div class="note">packet readiness · blocker · window</div></div>
        <div class="board">
          ${boardReadout().map((packet) => `<article class="pcard">
...
export function renderVerification() {
  return layout(
    "Claims vs Reality Engine — Verification",
    "/verification",
    `<section class="section">
        <div class="sh"><h2>Verification</h2><div class="note">board-safe claims only</div></div>
        <div class="stack">
          ${verification().map((item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface stays honest about offline exports, synthetic sample data, and board-ready claim posture.</p></div>`).join("")}
        </div>
      </section>`
  );
}
...
export function renderDocs() {
  return layout(
    "Claims vs Reality Engine — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/claim-audit</code>, <code>/proof-gaps</code>, <code>/board-readout</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/claim-audit</code>, <code>/api/proof-gaps</code>, <code>/api/board-readout</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline claim analysis</div><p><code>npx claims-vs-reality-engine fixtures/claims-vs-reality-clean.json --format summary</code> renders the same posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
